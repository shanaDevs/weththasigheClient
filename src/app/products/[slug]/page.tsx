...
import { ProductDetailSkeleton, OrderRequestDialog } from '@/components/products';
...
export default function ProductDetailPage() {
  ...
  const { addToCart, updateQuantity, isLoading: cartLoading } = useCartStore();
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-6">
            ...
            {product.taxEnabled && (
              <p className="text-sm text-slate-500">
                +{product.taxPercentage}% GST ({formatPrice((currentPrice * parseFloat(product.taxPercentage)) / 100)} per unit)
              </p>
            )}
              ...
              <div className="flex gap-3">
                ...
                {product.isMaxOrderRestricted && (
                  <Button
                    variant="outline"
                    size="lg"
                    className="h-14 px-6 text-emerald-600 border-emerald-200 hover:bg-emerald-50 font-bold"
            ...
            {product.isMaxOrderRestricted && (
              <div className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100/50 flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs text-emerald-800 font-bold uppercase tracking-wider mb-0.5">Bulk Ordering Available</p>
                  <p className="text-xs text-emerald-700/80 leading-relaxed font-medium">
                    Standard accounts are limited to <span className="font-bold underline decoration-emerald-300">{product.maxOrderQuantity} units</span> per order. For higher volumes, please use the request button above.
                  </p>
                </div>
              </div>
            )}
          ...
          <OrderRequestDialog
            product={product}
            open={showOrderRequestDialog}
            onOpenChange={setShowOrderRequestDialog}
          />
              ...
              <CardContent className="p-6 space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">On orders above {formatPrice(settings?.free_shipping_threshold || 5000)}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600 mt-0.5" />
